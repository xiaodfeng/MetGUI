...
options(guiToolkit="RGtk2")
  ...
  ####Set GUI parameters####
  setwd("D:/github/GUI")
F_Capture<-function(Code){capture.output(Code,file = "Output.log",type="message", append = TRUE)}
  ...
  widgets <- list()
  win <- gwindow("Window", visible=FALSE, 
  handler = function(h, ...) {
    gtkMainQuit() # stop main loop when windows is closed.
  }
  )
  # win = gWidgets2::gwindow("msGUI", expand = FALSE, fill = FALSE, height = 800, width=1000)
  ...
  svalue(nb) <- 1
  ####ImportData####
  gp_ImportData = ggroup(horizontal = FALSE, cont = nb, label = "ImportData")
  ...
  lo_Dataset[1,1] <- widgets$bt_folder  <-gbutton("Select dataset folder", handler = function(h,...) {
    ...
    if(length(widgets$my_path)==0) {galert('Please Select the folder to be Processed',title = "File Selection Problems",delay = 1)}
    else {
      ...
      for (i in 1:length(cdffiles)){
        ...
        lo_Dataset[i+2,11]<-ed_group[i]<<-gedit(text = "con", width = 4)
      }
    }
  })
  tooltip(widgets$bt_folder) <- "Select the folder for all the raw data to be processed"

  lo_Dataset[2,1] <- widgets$bt_group <- gbutton("Set Group", handler = function(h,...) {
    smp_grp <- rep("con", length(cdffiles))
    for (i in 1:length(cdffiles)){
  ...
  tooltip(widgets$bt_group) <- "Set Group information of all the loaded files"
  ...
  lo_Design[1,1] <- widgets$bt_importdata   <- gbutton("Import data", handler = function(h,...) {
  ...
  lo_Design[2,1] <- widgets$bt_plotTIC <- gbutton("Plot TIC", handler = function(h,...) {
    F_Delete_Ggraphics()
  ...
  lo_Design[3,1] <- widgets$bt_selectexportfolder <-  gbutton("Select export folder", handler = function(h,...) {
  ...
  lo_Design[4,1] <- widgets$bt_export <- gbutton("export", handler = function(h,...) {
  ...
  lo_Parameters[1,1] <- widgets$bt_selectparameterfile <- gbutton("Select Parameter File", handler = function(h,...) {
  ...
  ####PeakDetection####
  gp_PeakDetection = ggroup(horizontal = FALSE, cont = nb, label = "PeakDetection")
  ...
  lo_MatchedFilter[1,2]<- widgets$ed_binSize<-gedit(text = "0.04", width = 4)
  tooltip(widgets$ed_binSize)<-"binSize. Default 0.1. Specifying the width of the bins/slices in m/z dimension."

  
  lo_MatchedFilter[2,1] <-glabel("fwhm")
  lo_MatchedFilter[2,2]<-widgets$ed_fwhm<-gedit(text = "5", width = 4)
  tooltip(widgets$ed_fwhm)<-"fwhm. Default 30. specifying the full width at half maximum of matched filtration gaussian model peak, in seconds"

  
  lo_MatchedFilter[3,1] <-glabel("max")
  lo_MatchedFilter[3,2]<-widgets$ed_max<-gedit(text = "10", width = 4)
  tooltip(widgets$ed_max)<-"max. Default 5. representing the maximum number of peaks that are expected/will be identified per slice.“                                                                         "

  lo_MatchedFilter[4,1] <-glabel("snthreshMatchedFilter")
  lo_MatchedFilter[4,2]<-widgets$ed_snthreshMatchedFilter<-gedit(text = "6", width = 4)
  tooltip(widgets$ed_snthreshMatchedFilter)<-"snthresh. Default 10. defining the signal to noise cutoff to be used in the chromatographic peak detection step."  

  
  lo_MatchedFilter[5,1] <-glabel("steps")
  lo_MatchedFilter[5,2]<-widgets$ed_steps<-gedit(text = "4", width = 4)
  tooltip(widgets$ed_steps)<-"steps. Default 2.  Defining the number of bins to be merged before filtration (i.e. the number of neighboring bins that will be joined to the slice in which filtration and peak detection will be performed)."
                                                                         
  lo_MatchedFilter[6,1] <-glabel("mzdiff")
  lo_MatchedFilter[6,2]<-widgets$ed_mzdiff<-gedit(text = "0.008", width = 8)
  tooltip(widgets$ed_steps)<-"mzdiff. Default mzdiff = 0.8 - binSize * steps=0.6. Defining the minimum difference in m/z for peaks with overlapping retention times"
  ...
  lo_MatchedFilter[7,1]<-gbutton("Run", icon="help", handler = function(h,...) {
    ...
    if (binSizeValue==-1) {binSizeValue<<-as.numeric(svalue(widgets$ed_binSize))}
    if (fwhmValue==-1) {fwhmValue<<-as.numeric(svalue(widgets$ed_fwhm))}
    if (maxValue==-1) {maxValue<<-as.numeric(svalue(widgets$ed_max))}
    if (snthreshValue==-1) {snthreshValue<<-as.numeric(svalue(widgets$ed_snthreshMatchedFilter))}
    if (stepsValue==-1) {stepsValue<<-as.numeric(svalue(widgets$ed_steps))}
    if (mzdiffValue==-1) {mzdiffValue<<-as.numeric(svalue(widgets$ed_mzdiff))}  
  ...
  lo_CentWave[1,2]<-widgets$ed_ppm<-gedit(text = "10", width = 4)
  ...
  lo_CentWave[2,2]<-widgets$ed_peakwidthMin<-gedit(text = "2.4", width = 4)
  ...
  lo_CentWave[4,2]<-widgets$ed_snthreshCentWave<-gedit(text = "10", width = 4)
  ...
  lo_CentWave[5,2]<-widgets$ed_noise<-gedit(text = "1000000", width = 4)
  ...
  lo_CentWave[6,1]<- gbutton("Run",  handler = function(h,...) {
    ...
    if (ppmValue==-1) {ppmValue<<-as.numeric(svalue(widgets$ed_ppm))}
    
    peakwidthMinValue<<-as.numeric(as.character(Parameter[Parameters=="peakwidthMin",Values]))
    if (peakwidthMinValue==-1) {peakwidthMinValue<-as.numeric(svalue(widgets$ed_peakwidthMin))}
    ...
    if (snthreshCentWaveValue==-1) {snthreshCentWaveValue<<-as.numeric(svalue(widgets$ed_snthreshCentWave))}
    noiseValue<<-as.numeric(as.character(Parameter[Parameters=="noise",Values]))
    if (noiseValue==-1) {noiseValue<-as.numeric(svalue(widgets$ed_noise))}
    cwp <- CentWaveParam(
  ...
  ####RTAlignment####
  gp_RTAlignment = ggroup(horizontal = FALSE, cont = nb, label = "RTAlignment")
  ...
  lo_Obiwarp[2,1] <- widgets$bt_RTAlignment <- gbutton("Run", handler = function(h,...) {
    ...
    xdata1 <<- adjustRtime(xdata1, param = ObiwarpParam(subset = 4:6))# binSize defining the bin size (in mz dimension) to be used for the profile matrix generation. 
    #xdata1 <<- adjustRtime(xdata1, param = ObiwarpParam(binSize =as.numeric(binSizeRTValue))) 
    print("Obiwarp RT Alignment Done!")
  })
  tooltip(widgets$bt_RTAlignment)<-"Obiwarp RT Alignment method performs retention time adjustment using the Obiwarp method. It supports alignment of multiple samples by aligning each against a center sample. The alignment is performed directly on the profile-matrix. binSize defining the bin size (in mz dimension) to be used for the profile matrix generation."
  ...
  lo_Effect[1,1] <-  widgets$bt_TIC <- gbutton("TIC", handler = function(h,...) {
    F_Delete_Ggraphics()
    ...
    plot(chromatogram(xdata1), col = group_colors[name],peakType = "none") #,main="After retention time alignment"
    legend("topright",legend=name,col=group_colors[name], lty=1, cex=0.8) #,box.lty=0
  ...
  lo_Effect[2,2] <- rtMinAlign <- gedit(text = "125", width = 6)
  lo_Effect[3,1] <- glabel(paste("rtMax (<",round(RangeRT[2])," s)",sep=""))  
  lo_Effect[3,2] <- rtMaxAlign <- gedit(text = "145", width = 6)
  lo_Effect[4,1] <- glabel(paste("mzMin (>",round(RangeMZ[1]),")",sep=""))  
  lo_Effect[4,2] <-  mzMinAlign <- gedit(text = "364.3", width = 6)
  lo_Effect[5,1] <- glabel(paste("mzMax (<",round(RangeMZ[2]),")",sep=""))  
  lo_Effect[5,2] <-  mzMaxAlign <- gedit(text = "364.4", width = 6)
  lo_Effect[6,1] <-  widgets$bt_ExtractedIonChromatogram <- gbutton("Extracted ion chromatogram", handler = function(h,...) {
    F_Delete_Ggraphics()
    par(mfrow = c(2, 1), mar = c(4.5, 4.2, 1, 0.5))
    ##before time alignment
    ...
    legend("topright",legend=name,col=group_colors[name], lty=1, cex=0.8) #,box.lty=0
    print("Plot Extracted ion chromatogram Done!")
  })
  tooltip(widgets$bt_ExtractedIonChromatogram) <- "Plot the EIC before and after retention time alignment"

  ####Grouping####
  gp_Grouping = ggroup(horizontal = FALSE, cont = nb, label = "Grouping")
  ...
  lo_Density[4,1] <-  widgets$bt_DensityGrouping <- gbutton("Run", handler = function(h,...) {
  ...
  lo_XIC[5,1] <- widgets$bt_MassTrace <- gbutton("Mass trace", horizontal=FALSE, handler = function(h,...) {
    F_Delete_Ggraphics()
    xdata1 %>%
  ...
  tooltip(widgets$bt_MassTrace) <- "Plot mass trace of the raw data. The input MZ and RT value should within the dataset range."
 
  lo_XIC[6,1] <- widgets$bt_PlotEIC <- gbutton("Plot EIC", horizontal=FALSE, handler = function(h,...) {
    F_Delete_Ggraphics()
  ...
  tooltip(widgets$bt_PlotEIC) <- "Plot Extracted ion chromatogram of the raw data. The input MZ and RT value should within the dataset range."
  ...
  lo_GroupingEffect[3,1] <- widgets$bt_PlotGrouping <-  gbutton("PlotGrouping", handler = function(h,...) {
    F_Delete_Ggraphics()
  ...
  ####FillMissing####
  gp_FillMissing = ggroup(horizontal = FALSE, cont = nb, label = "FillMissing")
  ...
  lo_FillMissingDefault[1,1] <- widgets$bt_FillMissing<- gbutton("Run", handler = function(h,...) {
  ...
  lo_IsotopeFilteration[2,1] <- widgets$bt_Isotope <- gbutton("Run", handler = function(h,...) {
  ...
  lo_RemoveDup[3,1] <- widgets$bt_RemoveDup <-  gbutton("Run", handler = function(h,...) {
  ...
  lo_Scatter[2,1] <- widgets$bt_Scatter <-  gbutton("Plot", handler = function(h,...) {
    ###figure
  ...
  ####FilterNormalization####
  gp_FilterNormalization = ggroup(horizontal = FALSE, cont = nb, label = "FilterNormalization")
  lo_FilterNormalization <- glayout(cont = gp_FilterNormalization)
  ##Set Filteration gframe in FilterNormalization tab
  lo_FilterNormalization[1,1] <- gf_Filteration<-gframe("Filteration",expand=T)
  lo_Filteration<-glayout(cont=gf_Filteration)
  lo_Filteration[1,1]<- widgets$bt_Initialize  <-gbutton("Initialize", handler = function(h,...) {
  ...
  lo_Filteration[2,1]<-widgets$bt_SanityCheck<-gbutton("SanityCheck", handler = function(h,...) {
  ...
  tooltip(widgets$bt_SanityCheck) <- "SanityCheckData is used for data processing, and performs a basic sanity check of the uploaded content, ensuring that the data is suitable for further analysis. The function will return a message if the data has successfully passed the check and is deemed suitable for further analysis. If it fails, the function will return a 0. The function will perform the check directly onto the mSet$dataSet object, and must be performed immediately after reading in data. The sanity check function evaluates the accuracy of sample and class labels, data structure, deals with non-numeric values, removes columns that are constant across all samples (variance = 0), and by default replaces missing values with half of the original minimal positive value in your dataset.Before data analysis, a data integrity check is performed to make sure that all the necessary information
           ...
           "
  lo_Filteration[3,1]<-widgets$bt_ReplaceMissingValue<-gbutton("ReplaceMissingValue", handler = function(h,...) {
    FilterationCheck<<-FilterationCheck+1
  ...
  lo_Filteration[4,1]<-widgets$bt_FilterVariable<-gbutton("FilterVariable", handler = function(h,...) {
  ...
  lo_FilterNormalization[1,2] <- gf_Normalization<-gframe("Normalization",expand=T)
  lo_Normalization<-glayout(cont=gf_Normalization)
  lo_Normalization[1,1]<-glabel("normalization method")
  lo_Normalization[1,2:15]<-widgets$cb_rowNorm<- gcombobox(c("QuantileNorm","MedianNorm","ProbNormT","ProbNormF","CompNorm","SumNorm","SpecNorm"),expand=T)
  ...
  lo_Normalization[2,2:15]<-widgets$cb_transNorm<- gcombobox(c("NULL","LogNorm","CrNorm"))
  ...
  lo_Normalization[3,2:15]<-widgets$cb_scaleNorm<- gcombobox(c("AutoNorm","MeanCenter","ParetoNorm","RangeNorm"))
  ...
  lo_Normalization[4,1]<- widgets$bt_Normalization<- gbutton("Run",do.functions=FALSE, handler = function(h,...) {
    mSet<<-PreparePrenormData(mSet)
    mSet<<-Normalization(mSet, rowNorm=svalue(widgets$cb_rowNorm), 
                         transNorm=svalue(widgets$cb_transNorm), 
                         scaleNorm=svalue(widgets$cb_scaleNorm), 
  ...
  # lo_Normalization[5,1]<- widgets$bt_PlotNormalization<- gbutton("Plot Normalization Density", handler = function(h,...) {
  ...
  ####Statistics####
  gp_Statistics = ggroup(horizontal = FALSE, cont = nb, label = "Statistics")
  lo_Statistics <- glayout(cont = gp_Statistics)
  ##Set Volcano button in Statistics tab
  lo_Statistics[1,1:2] <- gf_Volcano<-gframe("Volcano Plot",expand=T)
  lo_Volcano<-glayout(cont=gf_Volcano)
  lo_Volcano[1,1]<-"Fold change threshold"
  lo_Volcano[1,2]<-ed_FCThreshold<-gedit(text = "2", width = 4)  
  ...
  lo_Volcano[2,1]<-"non-parametric test"
  lo_Volcano[2,2:8]<-cb_nonpar<-gcombobox(c("FALSE","TRUE"))
  
  lo_Volcano[3,1]<-"adjusted p-value (FDR) cutoff"
  lo_Volcano[3,2]<-ed_threshp<-gedit(text = "0.05", width = 4) 
  
  lo_Volcano[4,1]<-"paired"
  lo_Volcano[4,2:8]<-cb_paired<-gcombobox(c("FALSE","TRUE"))
  ...
  lo_Volcano[5,1:2] <- gbutton("Run Volcano Plot", handler = function(h,...) {
  mSet<<-Volcano.Anal(mSet, fcthresh=as.numeric(svalue(ed_FCThreshold)),
                        ...
                        cmpType=1,equal.var=TRUE, pval.type="fdr")
  ...
  lo_PCA[1,1]<- widgets$bt_PCA <- gbutton("RunPCA", handler = function(h,...) {
    mSet<<-PCA.Anal(mSet)
  ...
  lo_PCA[2,1]<- widgets$bt_PlotPCASummary <- gbutton("PlotPCASummary", handler = function(h,...) {
  ...
  lo_PLSDA[1,1]<- widgets$bt_PLSDA <- gbutton("RunPLSDA", handler = function(h,...) {
  ...
  lo_PLSDA[2,1]<- widgets$bt_PlotPLSSummary <-gbutton("Plot PLS Summary", handler = function(h,...) {
    F_Delete_Ggraphics()
    par(mar=c(0,0,0,0)) 
    plot.new()
  ...
  lo_Statistics[3,1] <-gf_Report<- gframe("PreparePDFReport",expand=T)
  ...
  ####Identification####
  gp_Identification = ggroup(horizontal = FALSE, cont = nb, label = "Identification")
  ...
  lo_HomeBuilt[3,1]<- widgets$bt_HomeBuilt <- gbutton("Run", handler = function(h,...) {
  ...
  tooltip(widgets$bt_HomeBuilt) <- "Home Built Identification only use MS1 for the identificaiton"
  ...
  lo_Metfrag[6,1] <- widgets$bt_Metfrag  <- gbutton("Run", handler = function(h,...) {
