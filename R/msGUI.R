#' @name msGUI
#' @title msGUI for software
#' @description The msGUI software.
...
#' @details  RGTK2 and GTK+ are required for msGUI. We recommend the 64-bit
...
msGUI <- function() {
  ...
  Database <<- data.table(read.csv("inst/extdata/Database/lipidmaps2017Short.csv"))
  ...
  val$binSize = val$fwhm = val$max = val$snthresh = val$steps = val$mzdiff = val$ppm = val$peakwidthMin = val$peakwidthMax = val$snthreshCentWave = val$noise = val$binSizeRT = val$bw = val$binSizeGrouping = val$ppmIsotope = val$ppmWindow = val$rtWindow = val$Database = val$DatabaseSearchRelativeMassDeviation = val$FragmentPeakMatchAbsoluteMassDeviation = val$FragmentPeakMatchRelativeMassDeviation = val$ppmHomeBuilt = -1
    # win <- gwindow("msGUI", visible=FALSE,
  #                handler = function(h, ...) {
  #                  gtkMainQuit() # stop main loop when windows is closed.
  #                }
  # )
  win <- gWidgets2::gwindow("msGUI", expand = FALSE, fill = FALSE, height = 800, width = 800)
  ...
  lo_ImportData[1, 1] <- gf_Dataset <- gframe("Spectra data preparation", horizontal = F, expand = TRUE)
  ...
  lo_Dataset[3, 1] <- widgets$bt_plotTIC <- gbutton("Plot TIC", handler = function(h, ...) {
    # F_Delete_Ggraphics()
  ...
  lo_Dataset[3, 2] <- widgets$bt_export <- gbutton("Export description", handler = function(h, ...) {
  ...
  lo_ImportData[1, 3] <- gf_Processing <- gframe("Spectra data pre-processing", horizontal = F, expand = TRUE)
  ...
  #### Isotope filtration ####
  lo_ImportData[2, 1] <- gf_Isotope <- gframe("Isotope filtration", horizontal = F, expand = TRUE)
  ...
  #### Identification####
  lo_ImportData[2, 2] <- gf_Identification <- gframe("Identification", horizontal = F, expand = TRUE)
  ...
  lo_Identification[1, 1:2] <- gbutton("Link MS1 with MS2", handler = function(h, ...) {
    ...
    cdfs <<- dir("d:/github/GUI/input/NormalVSHYP", full.names = TRUE,recursive = TRUE)
    pa <<- purityA(cdfs)
  ...
  lo_Identification[3, 1:2] <- gbutton("Create library database", handler = function(h, ...) {
    ...
    l_dbPthValue <- "d:/github/GUI/msGUI/input/MoNA-export-LC-MS-MS_Spectra.sqlite"
  ...
  lo_ImportData[2, 3] <- gf_Statistics <- gframe("Statistics and visualization", horizontal = F, expand = TRUE)
  ...
  lo_Statistics[10,2]<- ed_ReportName<-gedit(text = "msGUI", width = 4) 
  ...
  # addSpring(gp_About)
  gtext("msGUI is a function set with Gui to process mass spectrometry data", font.attr = list(size = "xx-large", family = "monospace"), expand = TRUE, fill = TRUE, container = gp_About)
  #### Plot tab####
